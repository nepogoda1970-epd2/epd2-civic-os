...
def validate_lock(lock: dict) -> None:
    ...
    if set(p3.get("required_capabilities") or []) != PILOT03_REQUIRED_CAPABILITIES:
...
def infer_stage_from_filename(name: str) -> str | None:
    ...
    return None if not match else f"PILOT-0{match.group(1)}"
...
def read_manifest_from_zip(zip_path: Path) -> tuple[dict, str, list[str]]:
    with zipfile.ZipFile(zip_path) as zf:
        ...
        if manifest_name not in names:
            fail(f"missing mandatory scope manifest {SCOPE_MANIFEST}; PILOT-03+ candidates fail closed")
        try:
...
def validate_candidate(zip_path: Path, lock: dict) -> None:
    ...
    if stage_title != FROZEN_STAGES[stage_id]:
        fail(f"scope mismatch for {stage_id}: expected {FROZEN_STAGES[stage_id]!r}, got {stage_title!r}")
    ...
    if stage_id == "PILOT-03":
        ...
        if pred.get("filename") != PILOT02_C4_FILENAME or pred.get("sha256") != PILOT02_C4_SHA256:
            fail("PILOT-03 candidate does not bind to accepted PILOT-02 C4 predecessor")
        if set(manifest.get("required_capabilities") or []) != PILOT03_REQUIRED_CAPABILITIES:
