...
def read_manifest_from_zip(zip_path: Path) -> tuple[dict[str, Any], str, list[str]]:
    with zipfile.ZipFile(zip_path) as zf:
        ...
        try:
            manifest: dict[str, Any] = json.loads(zf.read(manifest_name).decode("utf-8"))
        except Exception as exc:
            fail(f"invalid scope manifest: {exc}")
        return manifest, root, names


def validate_candidate(zip_path: Path, lock: dict[str, Any]) -> None:
