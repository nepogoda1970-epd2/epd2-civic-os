...
| INFRA01-HI-03 mandatory checks cannot disappear          | EXECUTED AND PASS | machine-readable registry, 42 governed checks, manifest accountability for each; `MANDATORY_CHECK_MISSING` (mutation M08); harness files pinned in required-path registry                                                                                            |
...
| INFRA01-HI-05 adversarially tested validator             | EXECUTED AND PASS | 16/16 mutation classes detected by 16 distinct detector codes (`test_infra01_mutation_suite.py`, executed inside `backend.pytest`)                                                                                                                                   |
...
| verify-backend          | `backend.pytest`                     | EXECUTED AND PASS | pytest: 5906 passed                    |
...
42/42 governed checks `EXECUTED AND PASS`; 0 `FAIL`, 0 `BLOCKED`, 0
`NOT APPLICABLE BY GOVERNED RULE`. Positive executed-test evidence: pytest
5906 passed (full suite) plus 157 passed (explicit adversarial/security
re-execution); node:test 3 (epd2-types) + 41 (web-shell); vitest 23 render
tests; Playwright 135 browser, 63 accessibility, 15 visual. Counts are
parsed from the captured logs and re-verified by the evidence validator;
run identity, per-check log SHA-256, freeze tree digest and the final
archive SHA-256 are in the delivered evidence bundle.
