...
- `scripts/check_repository.py` has never listed `docs/adr/ADR-026` через
  `ADR-037` (собственные ADR PACK-07 и PACK-08), хотя сами файлы
  присутствуют в `docs/adr/`. PACK-09 добавил только свои ADR-038…ADR-042
  и намеренно не расширял этот пробел и не «чинил» его задним числом —
  это остаётся отдельной задачей для владельца репозитория.
- Проверки CT-00-02 (unknown status), CT-00-03 (forbidden transition),
  CT-00-04 (idempotency), CT-00-06 (missing permission) и CT-00-07 (audit
  creation) для PACK-09 покрыты собственным тест-набором
  `services/compliance-service/tests/`, а не отдельными секциями в общих
  файлах `tests/contract/test_ct00_0*.py` — тем же решением, которое
  PACK-08 принял для `organization-service`. Расширенными для PACK-09
  общими файлами являются CT-00-01 (отдельный
  `test_ct00_01_pack09_schema_validation.py`), CT-00-08 и CT-00-09.
- PACK-09 намеренно не реализует коллегиальную (панельную) модель
  арбитража, правила кворума и автоматический вывод конфликта интересов
  из организационных связей: конфликт должен быть **объявлен**, система
  его не выводит (ADR-042).
- Полный frozen-инсталл (`uv sync --all-groups --frozen`, `npm ci`) и
  фронтенд-часть пайплайна (`npm run lint` / `typecheck` / `build`) в
  раунде PACK-09 выполнены не были — в песочнице отсутствовал сетевой
  доступ к реестрам пакетов. Подробности и точный список заблокированных
  команд — `docs/handover/PACK-09-IMPLEMENTATION-REPORT.md`, раздел 5.
