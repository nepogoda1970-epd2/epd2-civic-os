...
специфицированы 2026-07-27; управляющий ADR раунда — ADR-050,
...
(владелец: Finance Service, ADR-044) — сервисом, код которого настоящим
...
слоистая с единственным авторитетным денежным слоем (ADR-045): пробный
...
- `services/finance-service` **не существует** и настоящим разделом
  **не создаётся**; ни каталог, ни модуль, ни контракт не следуют из
  настоящего текста.
- Реализация требует **и** принятия ADR-044 через ADR-049, **и**
  принятия настоящего канонического содержания: ни одно из двух само
  по себе недостаточно, и ADR-050 как управляющий ADR раунда не
...
Добавлено версией канона 0.8.0 (ADR-044 через ADR-049, раздел 19f).
...
- **Канонический владелец** — `finance-service` (ADR-044) для каждого
