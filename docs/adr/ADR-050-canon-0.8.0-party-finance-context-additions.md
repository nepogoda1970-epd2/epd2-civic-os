# ADR-050: Canon minor-version addition: Party Finance & Financial Accountability Context (0.7.0 → 0.8.0)
...
ADR-044 (`finance-service` decomposition), ADR-045 (ledger, balanced
posting, correction model), ADR-046 (purpose-scoped financial party
references, lawful aggregation), ADR-047 (`Rechenschaftsbericht`
lifecycle, snapshot, authority semantics), ADR-048 (finance authority
separation, incompatible roles, independent audit) and ADR-049
...
preamble (`19f`'s opening paragraph) already names ADR-050 as
...
- Extends section 22 (ownership matrix) with **twenty-one** new rows,
  ...
  physically realized by one new service, `finance-service` (ADR-044,
...
round, gated on **both** ADR-044 through ADR-049 **and** this canon
...
later task, gated on this canon content **and** ADR-044 through
ADR-049, authorized by neither alone. **This ADR's status is
