...
*"In many verifiable election systems, it is assumed that a public bulletin
board exists for publishing these records. In practice, it has most often
taken the form of a simple web page"* `[E-07]`.

Belenios makes the same assumption and then documents what happens when it
is not met — its own caveats list *"Absence of a proper bulletin board"* as
a known limitation, noting that the board is in practice a webpage served
by the voting server and that a dishonest server *"may provide inconsistent
views to the participants"* `[E-15]`. Verificatum ships a board and calls
it a convenience: *"it is easy to replace"* `[E-32]`.
...
property for its control components — *"diverse design of the control
components and the independence of their operation and supervision"*
