...
| `RN-27` | **The guards are runtime checks, not an import boundary.** Nothing prevents a module from importing `DeterministicTestRandomSource` directly; what stops it being *used* is `RN-09`. No AST test asserts that production modules do not import it, although such a test exists for the verifier's import boundary |
