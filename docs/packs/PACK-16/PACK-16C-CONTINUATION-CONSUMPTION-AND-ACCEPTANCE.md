...
| **Redemption ↔ accepted-ballot linkage** | the two writes are separated in time, which is *better* for unlinkability | same | **the two writes are adjacent in time — the risk this option carries**, §5 | same |
...
| ID       | Control                                                                                                          |
| -------- | -------------------------------------------------------------------------------------------------------------------- |
| `CN-13`  | **The two writes are to different stores under different principals** (`CC-05`). Atomic *in effect* — the outcome is all-or-nothing — not a single transaction over a joined schema |
...
stores *and* to fine-grained infrastructure timing could narrow a
