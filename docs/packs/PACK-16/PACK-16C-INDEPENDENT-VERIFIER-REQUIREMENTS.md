...
| `IV-18` | **Check 20 is a public check on totals, not on pairings.** A public verifier counts leaves by class and compares with `E` and `E × K` from artefact 36. It cannot and must not check _which_ capability produced _which_ artefact                                                                                  |
