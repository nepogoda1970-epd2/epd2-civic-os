...
| ID      | Rule                                                                                                                                                                                                                                                                                                                                                                         |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `EB-22` | **`uv sync --all-groups --frozen` was executed on the network-enabled host, not in the build session.** The build session still has no package index, so it verifies the lock's _contents_ and the running library's _version_; it does not re-run the install. The distinction is stated because "the tests are green" and "the frozen install passes" are different claims |
...
| `EB-13` | **The parameter values never depended on either blocked path.** They are reconstructed **offline** from the published `ln 2` rule plus a recorded 279-bit offset, with `q`, `r` and `g` following in closed form. No file and no network is consulted. This is why the blockers cost provenance _traceability_ and not parameter _correctness_, and why the pin adds traceability rather than confidence in the numbers |
...
_Superseded by §1 and §2. Retained as the evidence that the blockers were
environmental. Commands were executed in the build environment on 2026-08-02 and
outputs are reproduced verbatim._
