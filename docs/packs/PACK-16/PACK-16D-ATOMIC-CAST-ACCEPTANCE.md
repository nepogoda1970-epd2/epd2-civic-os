...
| `AK-07` | Both orderings are inside one transaction, so neither is load-bearing for correctness under rollback. The ordering is load-bearing for what a fault-point crash *means* to a reviewer reading the trace |
