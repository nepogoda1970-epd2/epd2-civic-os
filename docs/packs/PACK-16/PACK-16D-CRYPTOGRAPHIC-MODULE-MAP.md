...
`EPD2-TESTONLY-NOTCONFORMANT-P4096-Q256` shares only the *dimensions* of
...
odd level means that two *different* leaf sequences can produce the same
...
| ID | Rule |
| -- | ---- |
| `CM-34` | **The tree is unkeyed: every hash uses `ZERO_KEY`.** These are public commitments over public artefacts. Secrecy of a leaf's *content* comes from the salt inside the leaf preimage, not from the tree |
