...
| `RB-09` | **`no_intermediate_tally` is one of the ten `IMMUTABLE_INVARIANTS`.** A feature flag whose normalised name merely _contains_ an invariant name is refused at startup, so `disable_no_intermediate_tally` fails the same way `no_intermediate_tally` does |
...
variable and no global. This is a deliberate choice about _where the
decision can be made_:
