...
This is a _reference_ verifier. An independent verifier written by another
...
| `RV-14` | The two tests are complementary: the `ast` check catches _reaching_ private state through an import, the symbol check catches _naming_ an identity concept even without importing anything                                        |
...
| `RV-66` | The full-record path also runs a per-leaf inclusion check inside `verify_batches`, but for a _complete_ opening that check is mathematically redundant with root recomputation — see `RV-72`                                         |
...
- **It is not an independent implementation.** It shares the repository's
  ...
  artefacts. It is independent of _private state_, which is what the two
...
- **It establishes no full interoperability.** The 23 internal stability
  ...
  agreement with another _complete_ ElectionGuard implementation is still
