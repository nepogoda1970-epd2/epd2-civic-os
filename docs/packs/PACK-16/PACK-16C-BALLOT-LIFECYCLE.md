...
| `encrypted` | *(no state)* | **Voter** | **Local diagnostic challenge** — checked locally, discarded, never submitted | none | no | none — client-local (`CH-39`) |
...
| `publication_disputed` | *(election-level)* | Election Board | Not remedied | `FM-16C-16` | **yes** | `publication.unrecoverable` |
...
| `BL-06`  | **`rejected` is absorbing for that envelope.** A rejected ballot is never later accepted; the voter submits a *new* ballot, and the capability was never consumed |
