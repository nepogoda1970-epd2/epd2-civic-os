...
one. `BB-28` records the same failure for mirrors — _"three mirrors in
three availability zones look like three mirrors and are one operator"_ —
...
| 6   | **The same HSM or HSM cluster**                                  | **hard** | _An HSM does not convert one administrator into a threshold_ (§4) | Separate devices under separate administration                          |
...
restores it. The election's _integrity_ claim may survive — the guardians
