...
discharge: neither the Independent Auditor (who verifies _this_ ceremony
against _this_ specification) nor the Security Administrator (who operates)
...
| --------------- | ----------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `R-01` × `R-08` | `△`         | **`✗`**  | Under `RS-16B-03` the Coordinator holds nothing, so the dual-control mitigation has nothing left to mitigate — but a Board member who also sequences the ceremony can determine _when_ a complaint window closes. Separated outright |
...
specified has to be _added_, in public, against `BR-09`, `GQ-13` and `CM-18`.
...
| `RS-16B-19` | A **federal-level shared roster** of qualified auditors, coordinators and candidate guardians may be maintained so that small units can draw on it. Drawing from a shared roster is declared, and two guardians drawn from the same roster are **not** thereby dependent — the roster confers nothing (`GI-02` applies to the _organisation_, not to the list) |
