...
| `SB-04` | **Production key ceremony.** A *reference* Feldman-VSS ceremony is executed, transcribed and verified by this code (`SB-13`), but it exchanges shares in-process with no authenticated channel, no ceremony script, no witnesses and no custody. None of that is a production ceremony (`OD-P16D-11`) | PACK-16B, PACK-17 |
...
commitments, so a wrong share is *detectable* rather than discovered at
...
| -- | ------------------ |
| `OD-P16D-02` | **Narrowed, not closed.** Two independent out-of-process oracles and primary-source parameters now exist, but there is no comparison against another *complete* ElectionGuard implementation. Interoperability remains unestablished |
...
outcomes for a follow-up item are exactly: *implemented in reference form*,
*partially implemented*, *test harness complete*, *deferred to PACK-17*,
*blocked pending external review*, *production hardening required*.
