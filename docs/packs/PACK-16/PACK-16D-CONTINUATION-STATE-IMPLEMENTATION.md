...
| `CS-03` | The dataclass is **frozen**. Every transition returns a *new* value via `dataclasses.replace`; nothing mutates in place, so a partially applied transition cannot exist |
