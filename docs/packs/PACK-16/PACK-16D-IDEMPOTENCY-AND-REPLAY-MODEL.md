...
silent misreading of a *different* request as the earlier one.
...
| `ID-09` | It is not a loophole in the entitlement limits either. The scope key governs *replay*; the number of accepted casts and challenges is governed by `ContinuationState`, which is checked separately in every transaction. Reusing a key across operations buys no extra entitlement |
...
**The defect.** The first implementation called `_replay()` *before*
