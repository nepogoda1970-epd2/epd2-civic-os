...
| 2 | A batch is released when its interval closes **and** it holds at least *k* assertions                                                   |
...
| 6 | Sub-*k* release writes `IssuanceCohortThresholdNotMet` to `AS-04` with the cohort-size **class**, never the exact size                   |
...
the *system* discloses; they do not change what a small group knows about
itself.
...
to know *that* a nonce was used; it never needs to know *what it produced*,
