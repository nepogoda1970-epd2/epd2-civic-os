...
| Residual risk       | Participation _intent_ is knowable on the identity side. Named, accepted, bounded to this side     |
...
| Preventive control  | Batching (120 s default), minimum cohort _k_ = 5, uniform release delay 30–300 s, cohort-of-one never released immediately, `cohort_wait_max` 3600 s, notification on the release schedule |
...
| Audit visibility    | `AS-03` only, recording _that_ a nonce was spent, never _what it produced_                                                                                                                                                                         |
| Correlation risk    | **Timing** — the residual named before the correction                                                                                                                                                                                              |
| Preventive control  | Set-not-map; no shared trace identifier; **coarsened timestamps (300 s default, ≥ 3600 s for small electorates)**; timing-class logging; **queued identity-side release**; **randomized voting-side minting delay 5–30 s**; minimum cohort _k_ = 5 |
