...
rec.setdefault('expected_state', {})['OPS-03'] = {
    'version': '0.1', 'status': 'ACCEPTED', 'implementation': 'CLOSED'
}
for region in rec.get('verification', {}).get('regions', []):
    if region.get('id') == 'ops03-not-accepted':
        region.clear()
        region.update({
            'id': 'ops03-accepted-closed',
            'must_include': 'OPS-03 ACCEPTED / CLOSED',
            'must_exclude': 'OPS-03 QUALIFICATION ELIGIBLE',
        })
