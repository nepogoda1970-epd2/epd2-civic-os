 ...
 * `'unsafe-inline'`: it would weaken the boundary for every script on a voting
 * origin in order to admit one framework script.  The right fix is a fresh
 * nonce per response, which Next applies to its own inline scripts and which
 * no injected script can guess.
 *
  ...
  matcher: [
    // Every document request. Static assets carry the config headers only.
    {
      ...
      missing: [
        ...
        // full page load — and a full page load discards the voter's ballot.
