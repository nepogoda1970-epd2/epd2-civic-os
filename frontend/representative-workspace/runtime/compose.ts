 ...
 * A browser gate asserts that absence by scanning the build for the fixture
 ...
 */
export const GOVERNED_TEST_FLAG = process.env.NEXT_PUBLIC_FRONT05_GOVERNED_TEST;

export function governedTestProfileEnabled(): boolean {
  return GOVERNED_TEST_FLAG === "1";
}

export async function resolveRepresentativeRuntime(): Promise<RepresentativeRuntime> {
  if (process.env.NEXT_PUBLIC_FRONT05_GOVERNED_TEST === "1") {
...
export function profileLabel(): string {
  return governedTestProfileEnabled()
    ? "Geprüftes Testprofil — keine echten Vorgänge"
