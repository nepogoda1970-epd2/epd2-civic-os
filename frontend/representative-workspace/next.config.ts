...
/**
 * WS-04 deployment profile.
 *
 * The headers below are the browser-boundary half of the isolation the stage
 * contract requires. They are asserted by tests/browser/front05.browser.spec.ts
 ...
 *
 * `Cache-Control: no-store` matters more here than on a public surface: this
 * origin renders confidential case material, and a shared or proxy cache
 * holding a mandate-scoped response is a cross-mandate disclosure.
 */
 ...
 * the emitted bundle rather than merely unreachable inside it. In this
 * workspace the fixture supplies case-shaped material, so an activated fixture
 * would put fabricated citizen cases in front of a representative.
 */
const GOVERNED_TEST_ENABLED =
  process.env.NEXT_PUBLIC_FRONT05_GOVERNED_TEST === "1";
...
const nextConfig: NextConfig = {
  ...
  // the processing path rather than leaving it unused but reachable.
