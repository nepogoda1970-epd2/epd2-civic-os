.PHONY: setup format format-check lint typecheck test test-python test-typescript \
        test-frontend test-browser check-repository build-frontend verify acceptance clean
...
# --- make acceptance ---
# The canonical INFRA-01 acceptance harness: one deterministic, fail-closed,
# evidence-producing acceptance run over the governed check registry,
# including freeze/package byte-identity proof and the sealed execution
# manifest. This is the single canonical acceptance entry point; `make
# verify` remains the quick sequential developer pipeline and is not an
# acceptance run. GitHub Actions (.github/workflows/infra01-acceptance.yml)
# invokes exactly this command.
acceptance:
