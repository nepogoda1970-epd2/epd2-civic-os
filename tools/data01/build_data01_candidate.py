...
FIXED_TS=1787184000  # 2026-08-20T00:00:00Z
...
def safe_extract_zip(z:Path,d:Path):
    with zipfile.ZipFile(z) as f:
        for i in f.infolist():
            ...
            if mode==0o120000: raise SystemExit(f'symlink forbidden {i.filename}')
        f.extractall(d)
...
def source_mode_map(c3:Path, patch:Path)->dict[str,int]:
    """Return canonical root-relative POSIX permission bits from governed inputs."""
    modes:dict[str,int]={"":0o755}
    with zipfile.ZipFile(c3) as z:
        names=[i.filename for i in z.infolist() if i.filename]
        roots={n.split('/')[0] for n in names}
        if len(roots)!=1: raise SystemExit(f'C3 mode map requires one root: {roots}')
        prefix=next(iter(roots))+'/'
        for i in z.infolist():
            if not i.filename or i.filename.rstrip('/')==prefix.rstrip('/'): continue
            if not i.filename.startswith(prefix): raise SystemExit(f'C3 path outside root: {i.filename}')
            rel=i.filename[len(prefix):].rstrip('/')
            if not rel: continue
            if i.is_dir():
                modes[rel]=0o755
            else:
                perm=(i.external_attr>>16)&0o7777
                modes[rel]=perm or 0o644
    with tarfile.open(patch,'r:gz') as t:
        for m in t.getmembers():
            rel=m.name
            while rel.startswith('./'): rel=rel[2:]
            rel=rel.rstrip('/')
            if not rel: continue
            if m.isdir(): modes[rel]=0o755
            elif m.isfile(): modes[rel]=m.mode & 0o7777 or 0o644
    return modes

def archive_mode(rel:str, is_dir:bool, modes:dict[str,int])->int:
    if is_dir: return 0o755
    return modes.get(rel,0o644)
...
def write_generated(root:Path, base:Path):
    ...
    for p in sorted((x for x in base.rglob('*') if x.is_file()), key=lambda x:x.relative_to(base).as_posix()):
        ...
        protected[rel]=sha(p)
    pm={
        'schema':'epd2.data01.predecessor-protection/1',
        'predecessor_filename':C3_NAME,
        'predecessor_sha256':C3_SHA,
        'declared_modified_predecessor_files':['package.json','SHA256SUMS.txt'],
        'declared_modified_predecessor_sha256':{n:sha(base/n) for n in ('package.json','SHA256SUMS.txt')},
        'protected_files':protected,
    }
    (root/'data').mkdir(exist_ok=True)
    (root/'data/predecessor-protected-sha256.json').write_text(json.dumps(pm,indent=2,sort_keys=True)+'\n')

    val={
        'schema':'epd2.data01.final-validation/1','status':'PENDING_LIVE_VALIDATION',
        'candidate_filename':TARGET_ZIP,'candidate_sha256':None,
        'predecessor_filename':C3_NAME,'predecessor_sha256':C3_SHA,
        'pb01_frozen_baseline_filename':C6_NAME,'pb01_frozen_baseline_sha256':C6_SHA,
        'live_execution':False,
        'failure_or_pending_reason':'sealed candidate requires independent exact-ZIP PostgreSQL 16 validation',
    ...
    bm=fmap(base); cm=fmap(root,{'SHA256SUMS.txt','DATA01_CHANGED_FILE_INVENTORY.json'})
    changes=[]
...
def main():
    ...
    modes=source_mode_map(c3,patch)
    shutil.rmtree(work,ignore_errors=True); (work/'c3').mkdir(parents=True); (work/'c6').mkdir();
    safe_extract_zip(c3,work/'c3'); safe_extract_zip(c6,work/'c6')
    base=one_root(work/'c3'); c6root=one_root(work/'c6')
    root=work/TARGET_ROOT; shutil.copytree(base,root,copy_function=shutil.copy2)
    # no generated/cache material from builder
    ...
    safe_extract_tar(patch,root)
    pbdir=root/'data/baselines/pb01'; pbdir.mkdir(parents=True,exist_ok=True)
    for n in PB01_FILES: shutil.copy2(c6root/n,pbdir/n)
    counts,entries=write_generated(root,base)
    for rel in ('data/predecessor-protected-sha256.json','DATA01_FINAL_VALIDATION.json','DATA01_DEVELOPER_VALIDATION_REPORT.md','DATA01_CHANGED_FILE_INVENTORY.json','SHA256SUMS.txt'):
        modes[rel]=0o644
    for n in PB01_FILES: modes[f'data/baselines/pb01/{n}']=0o644
    # Normalize archive metadata after bytes are finalized.
    ...
    # deterministic zip order/mode using Python, no extra fields
    with zipfile.ZipFile(output,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=6,strict_timestamps=True) as z:
        for p in [root,*sorted(root.rglob('*'),key=lambda x:x.relative_to(work).as_posix())]:
            ...
            if p.is_dir():
                perm=archive_mode(root_rel,True,modes)
                zi=zipfile.ZipInfo(rel.rstrip('/')+'/',(2026,8,20,0,0,0)); zi.create_system=3; zi.external_attr=((0o040000|perm)<<16)|0x10; zi.extra=b''; z.writestr(zi,b'',compress_type=zipfile.ZIP_STORED)
            else:
                perm=archive_mode(root_rel,False,modes)
                zi=zipfile.ZipInfo(rel,(2026,8,20,0,0,0)); zi.create_system=3; zi.external_attr=(0o100000|perm)<<16; zi.compress_type=zipfile.ZIP_DEFLATED; zi.extra=b''
