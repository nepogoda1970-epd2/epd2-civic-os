...
if current_master_sha != 'ac212cdd32c843a1403b069b51ea6e68a1f120ddadad414a50a0cbad35990e33':
    raise SystemExit('unexpected current-main Master SHA')
if 'FIR-OSS-007' not in current_master or OSS_MARKER not in current_master:
    raise SystemExit('current-main FIR-OSS-007 governance block not present')
...
if '## FIR-OSS-007' not in oss_block:
    raise SystemExit('FIR-OSS-007 announced but absent from exact current-main block')
...
for i, line in enumerate(lines[:15]):
    if line.startswith('**Maintenance copy:**'):
        lines[i] = ('**Maintenance copy:** V25 — canonical lossless lineage reconciliation (2026-08-29): '
                    'the independently reviewed accepted-maintenance + V23 union is preserved whole; '
                    f'the exact current-main V24 `FIR-OSS-007` addition is carried from commit `{CURRENT_MAIN_COMMIT}`; '
                    'no existing FIR is deleted, downgraded or renumbered by this maintenance repair. '
                    'See the V25 governance maintenance record and `docs/roadmap/EPD2_MASTER_V25_RECONCILIATION.json`.')
        break
else:
    raise SystemExit('maintenance-copy header not found')
merged_base = '\n'.join(lines).rstrip() + '\n\n---\n\n' + oss_block
...
required = ['FIR-UX-012','FIR-UX-013','FIR-AI-003','FIR-GOV-004','FIR-GOV-005','FIR-SEC-004','FIR-TRUST-002','FIR-TRUST-003','FIR-OSS-007']
for x in required:
    if x not in set(final_ids):
        raise SystemExit(f'required FIR missing: {x}')
MASTER.write_bytes(v25_master_bytes)

pcr = current_pcr.replace('**Updated:** 2026-08-27', '**Updated:** 2026-08-29', 1)
pattern = re.compile(r'^Current Master maintenance level established by project governance work: \*\*V24\*\*.*$', re.M)
replacement = ('Current Master maintenance level established by project governance work: **V25**, preserving the full accepted maintenance lineage '
...
pcr, n = pattern.subn(replacement, pcr, count=1)
if n != 1:
    raise SystemExit(f'expected exactly one current V24 Master claim; found {n}')
v24_marker = '**Documentation-only V24 governance update (2026-08-29):**'
start = pcr.find(v24_marker)
if start < 0:
    raise SystemExit('V24 PCR record not found')
end = pcr.find('\n\n', start)
if end < 0:
    raise SystemExit('V24 PCR paragraph end not found')
v25_pcr_record = ('\n\n**Governance reconciliation V25 (2026-08-29):** the canonical Master was losslessly reconciled upstream from '
                  f'current `main@{CURRENT_MAIN_COMMIT}` and the independently reviewed accepted-maintenance/V23 union. '
                  f'The exact V25 Master SHA-256 is `{v25_master_sha}`. The reconciliation preserves every FIR from both lineages, '
                  'carries `FIR-OSS-007` from current main, and records `missing_after_merge = []` and `duplicate_active_ids = []` in '
                  '`docs/roadmap/EPD2_MASTER_V25_RECONCILIATION.json`. No API, INFRA, OPS, CTRL, FRONT, SEC, PILOT or voting stage is accepted '
                  'or closed by this repair. `API-02 = ACTIVE / IN DEVELOPMENT`; `API-03 = PARALLEL_WORKING_PRESEAL_NOT_ACCEPTED`; '
...
if pcr.count('Current Master maintenance level established by project governance work: **V25**') != 1:
    raise SystemExit('PCR current V25 claim count != 1')
if pcr.count('**Governance reconciliation V25 (2026-08-29):**') != 1:
    raise SystemExit('PCR V25 reconciliation record count != 1')
...
recon = {
  'schema':'EPD2_MASTER_V25_RECONCILIATION/1','status':'PASS','date':'2026-08-29',
  'source_lineages':{
    'current_main':{'commit':CURRENT_MAIN_COMMIT,'parent':V23_COMMIT,'master_sha256_before_v25':current_master_sha,'pcr_sha256_before_v25':current_pcr_sha,'unique_fir_count':len(set(current_ids))},
    'v23':{'commit':V23_COMMIT,'master_sha256':V23_MASTER_SHA256},
    'independently_reviewed_c5_merge':{'api02_c5_archive_sha256':C5_ARCHIVE_SHA256,'merged_master_sha256':EXPECTED_C5_MERGED_SHA256,'accepted_api01_c5_archive_sha256':ACCEPTED_API01_C5_SHA256,'unique_fir_count':len(set(c5_ids))}},
  'result':{'maintenance_level':'V25','master_sha256':v25_master_sha,'pcr_sha256':v25_pcr_sha,'current_main_unique_firs':len(set(current_ids)),'c5_merged_unique_firs':len(set(c5_ids)),'union_unique_firs':len(expected_union),'v25_unique_firs':len(set(final_ids)),'added_from_current_main_vs_c5_merge':sorted(set(current_ids)-set(c5_ids)),'carried_only_from_c5_merge_vs_current_main':sorted(set(c5_ids)-set(current_ids)),'missing_after_merge':sorted(expected_union-set(final_ids)),'extra_after_merge':sorted(set(final_ids)-expected_union),'duplicate_active_ids':sorted(k for k,n in Counter(final_ids).items() if n>1),'required_presence':{x:(x in set(final_ids)) for x in required}},
  'execution_state_changed':False,'acceptance_or_closure_changed':False}
OUT_JSON.write_text(json.dumps(recon, indent=2, sort_keys=True) + '\n', encoding='utf-8')
if recon['result']['missing_after_merge'] or recon['result']['extra_after_merge'] or recon['result']['duplicate_active_ids'] or not all(recon['result']['required_presence'].values()):
